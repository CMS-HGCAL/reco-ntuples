...
private:
 ...
 // ----------member data ---------------------------
  ...
  edm::EDGetTokenT<reco::GenParticleCollection> _genParticles;
  
  TTree                     *tree;
  AEvent                    *event;
  AGenPartCollection        *agpc;
  AGenPartCollection        *agpc2;
  ...
  particleFilter(iConfig.getParameter<edm::ParameterSet>("TestParticleFilter"))
{
  ...
  _genParticles = consumes<reco::GenParticleCollection>(edm::InputTag("genParticles"));
  ...
  tree->Branch("genparticles","AGenPartCollection",&agpc2,16000,0); 
...
///////////////////////////////
  ...
  iEvent.getByToken(_genParticles, genParticleHandle);
  ...
  for(reco::GenParticleCollection::const_iterator genPart = genParticles->begin();genPart != genParticles->end();genPart++){
   ...
   /* std::cout<<"gen particle "
	     ...
	     <<std::endl;
*/
    agpc2->push_back(AGenPart(genPart->eta(),
			     genPart->phi(),
			     genPart->pt(),
			     genPart->energy(),
			     genPart->vx(),genPart->vy(),genPart->vz(),
			     genPart->pdgId(),
			     -99,-99,
			     false));
